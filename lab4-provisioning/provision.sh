...
sudo -E apt-key adv --keyserver hkp://keyserver.ubuntu.com:90 -- recv EA312927
...
# install the kerberos library which is needed to build the MongoDB Node.js driver during npm install
sudo -E apt-get install -y libkrb5-dev

# install pm2 utility for managing node servers
sudo -E npm install -g pm2 --loglevel error

# install the tsd utility for installing Visual Studio Code typings files gives statement completion and parameter hinting
